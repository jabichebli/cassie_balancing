...
switch robot_state
    case 'stage_0'
        ...
        % Threshold 0.05 handles noise but triggers fast
        if abs(a_com(1)) > 0.05
            state_change = true;
            robot_state = 'stage_1';
            
            % +X Accel -> Force from Right (Push Left) -> Step LEFT
            if a_com(1) > 0
                direction = 'left';
                fprintf('Detected Push LEFT (a=%.2f). Stepping LEFT.\n', a_com(1));
                
                % SWING: Left (Cols 1-2) | STANCE: Right (Cols 3-4)
                persist_params.stage1 = [ ...
                    [0.0921+persist_params.foot_dx;   0.1305+persist_params.foot_dy;     0.3], ... % L Target (Swing)
                    [-0.0879+persist_params.foot_dx;  0.1305+persist_params.foot_dy;     0.3], ... 
                    [0.0921;                         -0.1305-persist_params.foot_dy*1.2; -0.3], ... % R Fixed (Stance)
                    [-0.0879;                        -0.1305-persist_params.foot_dy*1.2; -0.3]];    
                
                % Target for Single Foot IK (Swing Left)
                ...
                % Constraint Mask: Fix RIGHT (3,4), Move LEFT (1,2)
                persist_params.mask = [0;0;1;1];
                
                % Gains: Left=Swing(High), Right=Stance(Low)
                persist_params.stage1_kp_left = 1700;  
                persist_params.stage1_kd_left = 300;   % <--- ADDED THIS (Was missing)
                persist_params.stage1_kp_right = 1400; 
                persist_params.stage1_kd_right = 300;  % <--- ADDED THIS (Was missing)

            % -X Accel -> Force from Left (Push Right) -> Step RIGHT (Original Working Case)
            else
                direction = 'right';
                fprintf('Detected Push RIGHT (a=%.2f). Stepping RIGHT.\n', a_com(1));
                
                % SWING: Right (Cols 3-4) | STANCE: Left (Cols 1-2)
                persist_params.stage1 = [ ...
                    [0.0921;                          0.1305+persist_params.foot_dy*1.2; -0.3], ... % L Fixed (Stance)
                    [-0.0879;                         0.1305+persist_params.foot_dy*1.2; -0.3], ... 
                    [0.0921+persist_params.foot_dx;  -0.1305-persist_params.foot_dy;      0.3], ... % R Target (Swing)
                    [-0.0879+persist_params.foot_dx; -0.1305-persist_params.foot_dy;      0.3]];    
                
                % Target for Single Foot IK (Swing Right)
                persist_params.p_target_ankle = [0.0921+persist_params.foot_dx; -0.1305-persist_params.foot_dy; 0.3];
                persist_params.p_target_toe   = [-0.0879+persist_params.foot_dx; -0.1305-persist_params.foot_dy; 0.3];
                
                % Constraint Mask: Fix LEFT (1,2), Move RIGHT (3,4)
    ...
    case 'stage_1'
        ...
        if current_bucket > last_ik_bucket
            ...
            % 3. Splicing (CRITICAL: Validated Indices)
            if strcmp(direction, 'left')
                % Left Leg Indices: HipRoll(7), HipYaw(9), HipPitch(11), Knee(13), Toe(19)
                idx_splice = [7, 9, 11, 13, 19];
            else
                % Right Leg Indices: HipRoll(8), HipYaw(10), HipPitch(12), Knee(14), Toe(20)
        ...
        % Low Pitch/Knee weights allow the Stance Leg to "adjust" (bend) under load
        ...
        if strcmp(direction, 'left')
            swing_z = min(p_feet(3, 1:2)); % Left
        else
            swing_z = min(p_feet(3, 3:4)); % Right
        end
        
        % Wait 0.5s before checking touchdown to allow leg lift
        if t > 0.5 && swing_z < 1e-3
    ...
    case 'stage_end'
        ...
        % Restored Stage End Weights (High Stiffness)
        w = [1, 1, 0.7, 0.6, 0.4];
        C = [w(:), w(:)].';
        total_weights = C(:);
        ...
        % fprintf("Damping Modifier: ");disp(damping_modifier);
