...
figure(1); 
    subplot(3,1,1);plot(t_vec, r_com) ;grid ; title('com positions x-y-z') ; legend('x','y','z') ;
...
figure(2) ; plot(t_vec, x_vec(:,1:3)) ; grid on ;
    title('Base (Pelvis) Translation') ; legend('x','y','z') ;

% Plot Base (Pelvis) Orientation
figure(3) ; plot(t_vec, x_vec(:,4:6)*180/pi) ; grid on ;
    title('Base (Pelvis) Orientation') ; legend('y','p','r') ;
